...
iOS User Notifications 最大的创新在于加入了以 Extension 形式提供给开发者和用户使用的 UNNotificationContentExtension，但是只支持能使用 3DTouch 的设备...。通知内容扩展需要新建一个 NotificationContent Target，之后只需在 viewcontroller 的中实现相应的接口，即可以对 app 的通知页面进行自定义扩展，扩展主要用于自定义 UI。
